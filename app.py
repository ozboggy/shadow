...
# Warn if no FR24/JS feed data: try OpenSky fallback
if not positions:
    st.warning("No FR24 data; falling back to OpenSky network.")
    # Determine auth for OpenSky
    if OSKY_USER and OSKY_PASS:
        auth = (OSKY_USER, OSKY_PASS)
        st.sidebar.write("Using OpenSky with credentials")
    else:
        auth = None
    ...
    try:
        ...
        states = []
    # Parse states: [icao24, callsign, ..., longitude index 5, latitude index 6, altitude 7, speed 9, track 10]
    for stt in states:
        lon = stt[5]
        lat = stt[6]
        if lat is None or lon is None:
