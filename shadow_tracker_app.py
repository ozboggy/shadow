...
if st.sidebar.button("Test Pushover"):
    ok = send_pushover("🔔 This is a Pushover test.")
...
if data_source == "ADS-B Exchange":
    if "api-auth" not in ADSB_HEADERS:
        st.error("Missing ADS-B Exchange API key.")
    else:
            ...
            if resp.status_code != 200:
                ...
                if not body:
                    ...
                    try:
                        ...
                        st.error(f"JSON parse error: {jde}. Raw: {body}")
        except Exception as e:
            st.error(f"Error fetching ADS-B Exchange: {e}")
elif data_source == "OpenSky":
    try:
        resp = requests.get(f"https://{OPENSKY_HOST}/api/states/all", auth=(OPENSKY_USERNAME, OPENSKY_PASSWORD), timeout=10)
