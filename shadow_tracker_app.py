...
# Alert thresholds (seconds before transit)
...
# Calculate sun and moon altitude
...
# Fetch live aircraft data via ADS-B Exchange
aircraft_list = []
if RAPIDAPI_KEY:
...
# Prepare shadow export logs
sun_export, moon_export = [], []

# Compute shadow trails & collect export points
...
if show_sun_lines:
    for ac, trail in sun_trails:
        ...
        layers.append(pdk.Layer(
            "ScatterplotLayer",
            data=pd.DataFrame([{"lat": slat, "lon": slon}]),
            get_position=["lon", "lat"],
