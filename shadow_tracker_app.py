...
with st.sidebar:
    ...
    # Aircraft count
    count = len(st.session_state.get('df_ac', []))
    st.markdown(f"**Tracked Aircraft:** {count}")
    # Sun/Moon heights
    ...
    st.markdown(
        f"<span style='color:{sun_col}'>Sun Altitude: {sa:.1f}°</span>",
        unsafe_allow_html=True
    )
    if ma is not None:
        moon_col = "green" if ma > 0 else "red"
        st.markdown(
            f"<span style='color:{moon_col}'>Moon Altitude: {ma:.1f}°</span>",
            unsafe_allow_html=True
        )
...
# Store for sidebar count
st.session_state['df_ac'] = aircraft_list
# DataFrame for map
df_ac = pd.DataFrame(aircraft_list)

# Prepare exports and alerts
sun_export, moon_export, alert_msgs = [], [], []
...
if not df_ac.empty:
    layers.append(pdk.Layer(
        ...
        get_position=["lon","lat"], get_fill_color=[0,128,255,255], get_radius=200, pickable=True
    ))
# Home boundary layer
layers.append(pdk.Layer(
    "ScatterplotLayer", data=pd.DataFrame([{"lat":CENTER_LAT,"lon":CENTER_LON}]),
    get_position=["lon","lat"], get_fill_color=[255,0,0,128], get_radius=alert_width, pickable=False
))
# Sun trails
if show_sun_lines:
    for ac, trail in sun_trails:
        layers.append(pdk.Layer(
            "PathLayer", data=[{"path":[(lon,lat) for lat,lon,_ in trail]}], get_path="path",
            get_color=[0,0,0], width_scale=(ac["alt"]/10000+1)*0.5, width_min_pixels=1, pickable=False
        ))
        slat, slon, _ = trail[0]
        layers.append(pdk.Layer(
...
if show_moon_lines:
    for ac, trail in moon_trails:
        layers.append(pdk.Layer(
            "PathLayer", data=[{"path":[(lon,lat) for lat,lon,_ in trail]}], get_path="path",
            get_color=[128,128,128], width_scale=ac["alt"]/10000+1, width_min_pixels=2, pickable=False
        ))
...
# Handle Test Alert (inject into alerts)
if test_alert:
    test_msg = "✈️ Test alert triggered!"
    if test_msg not in st.session_state.alerted:
        st.session_state.alerted.add(test_msg)
        alert_msgs.append(test_msg)

# Persistent on-screen alerts with single-play sound
def play_alerts(msgs):
    if 'played_alerts' not in st.session_state:
        st.session_state.played_alerts = set()
    for msg in msgs:
        st.warning(msg)
        if msg not in st.session_state.played_alerts:
            st.markdown(
                "<audio autoplay><source src='https://www.soundjay.com/buttons/sounds/beep-07.mp3' type='audio/mpeg'></audio>",
                unsafe_allow_html=True
            )
            st.session_state.played_alerts.add(msg)

play_alerts(alert_msgs)

# Test Pushover remains
