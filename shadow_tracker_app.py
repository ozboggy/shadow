...
with st.sidebar:
    ...
    st.markdown(f"Total aircraft: **{len(df_ac)}**")
    st.markdown(f"Military: **{df_ac['mil'].sum()}**")

# Compute shadows
sun_trails, moon_trails = [], []
if not df_ac.empty:
    if ephem:
        obs = ephem.Observer(); obs.lat, obs.lon = str(CENTER_LAT), str(CENTER_LON)
...
if track_sun and sun_trails:
    ...
    layers.append(pdk.Layer("PathLayer", df_sun, get_path="path", get_color=[50,50,50,255], width_scale=5, width_min_pixels=1, pickable=False))
    sun_cur = pd.DataFrame([{"lon": s["current"][0], "lat": s["current"][1]} for s in sun_trails])
    layers.append(pdk.Layer("ScatterplotLayer", sun_cur, get_position=["lon","lat"], get_fill_color=[50,50,50,255], get_radius=100, pickable=False))

# Moon shadows
if track_moon and moon_trails:
    df_moon = pd.DataFrame(moon_trails)
    layers.append(pdk.Layer("PathLayer", df_moon, get_path="path", get_color=[180,180,180,200], width_scale=5, width_min_pixels=1, pickable=False))
    moon_cur = pd.DataFrame([{"lon": m["current"][0], "lat": m["current"][1]} for m in moon_trails])
    layers.append(pdk.Layer("ScatterplotLayer", moon_cur, get_position=["lon","lat"], get_fill_color=[180,180,180,200], get_radius=100, pickable=False))
...
if not df_ac.empty:
    ...
    mil = df_ac[df_ac['mil']]
    if not civ.empty:
        layers.append(pdk.Layer("ScatterplotLayer", civ, get_position=["lon","lat"], get_fill_color=[0,128,255,200], get_radius=300, pickable=True, auto_highlight=True, highlight_color=[255,255,0,255]))
    if not mil.empty:
        layers.append(pdk.Layer("ScatterplotLayer", mil, get_position=["lon","lat"], get_fill_color=[255,0,0,255], get_radius=400, pickable=True, auto_highlight=True, highlight_color=[255,255,0,255]))
