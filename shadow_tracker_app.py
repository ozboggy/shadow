...
tile_style = st.sidebar.selectbox("Map Tile Style", ["OpenStreetMap", "CartoDB positron", "CartoDB dark_matter", "Stamen Terrain", "Stamen Toner"], index=0)
...
if st.sidebar.button("Send Test Push"):
    resp = requests.post(
        ...
        data={"user": PUSHOVER_USER_KEY, "token": PUSHOVER_APP_TOKEN,
              "message": f"Test from Shadow Tracker at {datetime.utcnow().isoformat()}UTC"}
    )
...
def fetch_opensky(lat, lon, radius):
    ...
    for s in states:
        ...
        try:
            cs=s[1].strip() or s[0]; lat_f=float(s[6]); lon_f=float(s[5])
            baro=float(s[7] or 0); vel=float(s[9]); hdg=float(s[10])
        except: continue
...
def fetch_adsb(lat, lon, radius):
    ...
    for ac in ac_list:
        try:
            cs=(ac.get("flight")orac.get("hex")or"").strip(); lat_f=float(ac.get("lat")); lon_f=float(ac.get("lon"))
            baro=float(ac.get("alt_baro") or 0); vel=float(ac.get("gs")orac.get("spd")or0)
...
def calculate_trail(lat, lon, baro, vel, hdg):
    ...
    for i in range(0,int(forecast_duration*60)+1,forecast_interval):
        ft=selected_time+timedelta(seconds=i);dist=vel*i
...
# Home shadow audio alert
home_alert=False

# Plot
for ac in aircraft_list:
    lat,lon,baro,vel,hdg,cs=ac.values()
    folium.map.Marker((lat,lon),icon=DivIcon(icon_size=(20,20),icon_anchor=(10,10),html=f'<i class="fa fa-plane" style="color:blue;transform:rotate({hdg-90}deg);transform-origin:center;font-size:20px"></i>'),popup=f"{cs}\nAlt:{baro}m\nSpd:{vel}m/s").add_to(fmap)
    ...
    if trail:
        ...
        fmap.add_child(line)
        folium.plugins.PolyLineTextPath(line,'▶',repeat=True,offset=10,attributes={'fill':'blue','font-weight':'bold','font-size':'6px'}).add_to(fmap)
        # Check if any shadow point over home
        for (s_lat,s_lon) in trail:
            d = math.hypot((s_lat-TARGET_LAT)*111111, (s_lon-TARGET_LON)*111111*math.cos(math.radians(TARGET_LAT)))
            if d <= HOME_ALERT_THRESHOLD_M:
                home_alert=True

# Render map
st_folium(fmap,width=900,height=600)

# Audio alert if condition met\if enable_audio and home_alert:
    beep=generate_beep()
