...
# Sidebar: map style & debug
st.sidebar.header("Display Settings")
map_theme      = st.sidebar.radio("Map Theme", ["Light", "Dark"], index=0)
debug_no_map   = st.sidebar.checkbox("Debug: No map tiles", False)

# Determine pydeck map_style
if debug_no_map:
    map_style = None
else:
    if map_theme == "Light":
...
sun_alt = get_altitude(CENTER_LAT, CENTER_LON, datetime.now(timezone.utc))
st.sidebar.markdown(f"**Sun altitude:** {sun_alt:.1f}°")
if ephem:
    obs = ephem.Observer()
    obs.lat, obs.lon, obs.date = str(CENTER_LAT), str(CENTER_LON), datetime.now(timezone.utc)
...
if ADSBEX_TOKEN:
    try:
        ...
        headers = {"x-rapidapi-key": ADSBEX_TOKEN, "x-rapidapi-host": "adsbexchange-com1.p.rapidapi.com"}
...
if not raw:
    ...
    raw = [
        {"lat": s[6], "lon": s[5], "alt": s[13] or 0.0, "track": s[10] or 0.0, "callsign": (s[1].strip() or s[0])}
...
if track_moon and ephem:
    for _, r in df.iterrows():
        ...
        for sec in range(0, FORECAST_INTERVAL_SEC * FORECAST_DURATION_MIN + 1, FORECAST_INTERVAL_SEC):
            ...
            if ma>0:
                mz=math.degrees(float(m.az)); d=r.alt/math.tan(math.radians(ma))
...
# Debug trails
if debug_trails:
    st.sidebar.write("Sun trail sample:", trails_sun[:1])
    st.sidebar.write("Moon trail sample:", trails_moon[:1])
...
for tr in trails_sun:
    for lon, lat in tr["path"]:
        if hav(lat, lon, CENTER_LAT, CENTER_LON) <= alert_width:
            alerts.append(tr["callsign"]); break
