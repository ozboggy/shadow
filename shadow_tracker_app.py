...
Load ephemeris for moon calculations for moon calculations

_eph = load('de421.bsp') _ts = load.timescale() _earth = _eph['earth'] _moon = _eph['moon']
...
sun_trails = [] moon_trails = [] if not df_ac.empty: for _, row in df_ac.iterrows(): cs = row['callsign'] # sun shadows if track_sun: path = [] lat0, lon0 = row['lat'], row['lon'] for i in range(0, FORECAST_INTERVAL_SECONDSFORECAST_DURATION_MINUTES+1, FORECAST_INTERVAL_SECONDS): ft = now + timedelta(seconds=i) # move plane dist = row['vel'] * i dlat = dist * math.cos(math.radians(row['hdg'])) / 111111 dlon = dist * math.sin(math.radians(row['hdg'])) / (111111 * math.cos(math.radians(lat0))) lat_i = lat0 + dlat lon_i = lon0 + dlon sun_alt = get_altitude(lat_i, lon_i, ft) sun_az = get_azimuth(lat_i, lon_i, ft) if sun_alt > 0: shadow_dist = row['alt'] / math.tan(math.radians(sun_alt)) sh_lat = lat_i + (shadow_dist/111111) * math.cos(math.radians(sun_az+180)) sh_lon = lon_i + (shadow_dist/(111111math.cos(math.radians(lat_i)))) * math.sin(math.radians(sun_az+180)) path.append([sh_lon, sh_lat]) if path: sun_trails.append({'path': path, 'callsign': cs}) # moon shadows if track_moon: moon_path = [] lat0, lon0 = row['lat'], row['lon'] for i in range(0, FORECAST_INTERVAL_SECONDSFORECAST_DURATION_MINUTES+1, FORECAST_INTERVAL_SECONDS): ft = now + timedelta(seconds=i) t = _earth + Topos(latitude_degrees=lat0, longitude_degrees=lon0) ts_t = _ts.utc(ft.year, ft.month, ft.day, ft.hour, ft.minute, ft.second) astrometric = t.at(ts_t).observe(_moon).apparent() alt_moon, az_moon, _ = astrometric.altaz() if alt_moon.degrees > 0: m_dist = row['alt'] / math.tan(math.radians(alt_moon.degrees)) mlat = lat0 + (m_dist/111111) * math.cos(math.radians(az_moon.degrees+180)) mlon = lon0 + (m_dist/(111111math.cos(math.radians(lat0)))) * math.sin(math.radians(az_moon.degrees+180)) moon_path.append([mlon, mlat]) if moon_path: moon_trails.append({'path': moon_path, 'callsign': cs})
