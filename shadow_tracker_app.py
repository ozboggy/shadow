...
if page_theme == "Dark":
    st.markdown(
        ...
        /* Text */
        .css-1d391kg, .css-ffhzg2, .css-18e3th9 { background-color: transparent; color: #EEEEEE; }
        </style>
...
# Fetch aircraft via ADS-B Exchange rapidAPI fallback to OpenSky
...
if not raw:
    ...
    try:
        ...
        states = []
    raw = [{"lat":s[6],"lon":s[5],"alt":s[13] or 0.0,"track":s[10] or 0.0,
            "callsign":(s[1].strip()or s[0])} for s in states if len(s)>=11]
    ...
    for _,r in df.iterrows():
        ...
        for sec in range(0,FORECAST_INTERVAL_SEC*FORECAST_DURATION_MIN+1,FORECAST_INTERVAL_SEC):
            ...
            if sa>0:
                ...
                path.append([r.lon + (d/(111111*math.cos(math.radians(r.lat))))*math.sin(math.radians(az+180)),
                             r.lat + (d/111111)*math.cos(math.radians(az+180))])
        if path: trails_sun.append({"callsign":r.callsign,"path":path})
if track_moon and ephem:
    for _,r in df.iterrows():
        ...
        for sec in range(0,FORECAST_INTERVAL_SEC*FORECAST_DURATION_MIN+1,FORECAST_INTERVAL_SEC):
            ...
            m=ephem.Moon(obs); ma,az=math.degrees(float(m.alt)),math.degrees(float(m.az))
            if ma>0:
                d=r.alt/math.tan(math.radians(ma))
                path.append([r.lon + (d/(111111*math.cos(math.radians(r.lat))))*math.sin(math.radians(az+180)),
                             r.lat + (d/111111)*math.cos(math.radians(az+180))])
        if path: trails_moon.append({"callsign":r.callsign,"path":path})
...
for tr in trails_sun:
    if any(hav(lat,lon,CENTER_LAT,CENTER_LON)<=alert_width for lon,lat in tr["path"]): alerts.append(tr["callsign"]);
...
if not df_safe.empty:
    layers.append(pdk.Layer("ScatterplotLayer",df_safe,get_position=["lon","lat"],get_color=[0,0,255,200],get_radius=80))
if not df_warn.empty:
    layers.append(pdk.Layer("ScatterplotLayer",df_warn,get_position=["lon","lat"],get_color=[255,0,0,200],get_radius=80))
if track_sun and trails_sun:
    layers.append(pdk.Layer("PathLayer",trails_sun,get_path="path",get_color=[255,215,0],width_scale=15,width_min_pixels=4))
if track_moon and trails_moon:
    layers.append(pdk.Layer("PathLayer",trails_moon,get_path="path",get_color=[100,100,100],width_scale=10,width_min_pixels=3))
# Home marker
...
if hide_map:
    ...
    st.pydeck_chart(pdk.Deck(layers=layers,initial_view_state=view,map_style=map_style),use_container_width=True)

# Onscreen alerts
if alerts and enable_onscreen:
    st.error("🚨 Shadow ALERT!"); st.audio("https://actions.google.com/sounds/v1/alarms/alarm_clock.ogg",autoplay=True)
for cs in alerts: st.write(f"✈️ {cs} — shadow predicted")
if not alerts: st.success("✅ No shadows predicted.")
